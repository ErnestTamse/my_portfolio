...
if(isset($_POST['change_about_picture'])){
    ...
    if(in_array($fileActualExt, $allowedExt)){         
        ...
        if($fileError == 0){                          
            ...
            $insert_data = mysqli_query($conn, "UPDATE user SET about_picture='$selected_image' WHERE user_id=$user_id");
            
            if($insert_data) {
                echo "<script>alert('About Picture Successfully Updated.')</script>";
...
<body style="background-color: grey;">
    ...
    <div data-aos="fade-right" class="container container-custom form-container w-50" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%)">
        ...
        <h1 class="fs-5 mt-3">Plese select your about picture</h1>
        <hr>
        <form class="d-flex flex-column" method="post" enctype="multipart/form-data">
            ...
            <button type="submit" class="btn mt-4" style="background-color: #262018; color: #DED1B7;" name="change_about_picture">Change about picture</button>
        </form>
